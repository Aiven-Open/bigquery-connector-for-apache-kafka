...
public class StorageApiBatchModeHandler {
  ...
  public StorageApiBatchModeHandler(
      StorageWriteApiBatchApplicationStream streamApi, BigQuerySinkTaskConfig config) {
    this.streamApi = streamApi;
  }

  /** Used by the scheduler to commit all eligible streams and create new active streams. */
  ...
  /**
   * Saves the offsets assigned to a particular stream on a table. This is required to commit
   * offsets sequentially even if the execution takes place in parallel at different times.
   *
   * @param tableName Name of tha table in project/dataset/tablename format
   * @param rows Records which would be written to table {tableName} sent to define schema if table
