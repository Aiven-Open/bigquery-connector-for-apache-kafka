...
public class KafkaKeyFieldNameValidator extends MultiPropertyValidator<BigQuerySinkConfig> {
  ...
  protected Optional<String> doValidate(BigQuerySinkConfig config) {
    //  Kafka key field name should be null if storage write API is used with upsert or delete enabled.
    ...
    if (config.useStorageWriteApi()) {
      if (StringUtils.isNotBlank(value) && upsertOrDelete) {
        return Optional.of(String.format(
              "%s may not be set if %s is set and either %s or %s are set.", BigQuerySinkConfig.KAFKA_KEY_FIELD_NAME_CONFIG,
               BigQuerySinkConfig.USE_STORAGE_WRITE_API_CONFIG, BigQuerySinkConfig.UPSERT_ENABLED_CONFIG, BigQuerySinkConfig.DELETE_ENABLED_CONFIG));
      }
    } else {
      if (StringUtils.isBlank(value) && upsertOrDelete) {
        return Optional.of(String.format(
              ...
              BigQuerySinkConfig.UPSERT_ENABLED_CONFIG, BigQuerySinkConfig.DELETE_ENABLED_CONFIG,
