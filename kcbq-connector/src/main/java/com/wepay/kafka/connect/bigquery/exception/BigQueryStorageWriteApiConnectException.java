...
/**
 * Exception Class for exceptions that occur while interacting with BigQuery Storage Write API, such
 * as login failures, schema update failures, and table insertion failures.
 */
public class BigQueryStorageWriteApiConnectException extends ConnectException {
  ...
  private static String formatRowErrors(String tableName, List<RowError> errors) {
    ...
    for (RowError error : errors) {
      builder.append(
          String.format(
              "\n [row index %d] (Failure reason : %s) ", error.getIndex(), error.getMessage()));
    }
  ...
  private static String formatRowErrors(String tableName, Map<Integer, String> errors) {
    ...
    for (Map.Entry<Integer, String> error : errors.entrySet()) {
      builder.append(
          String.format(
              "\n [row index %d] (Failure reason : %s) ", error.getKey(), error.getValue()));
    }
    return builder.toString();
  }
}
