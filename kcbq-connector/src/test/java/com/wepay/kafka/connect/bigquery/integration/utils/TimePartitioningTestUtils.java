...
public class TimePartitioningTestUtils {
  ...
  public static void produceRecordsWithTimestamps(
    ...
    try (Producer<Void, String> producer =
        new KafkaProducer<>(props, Serdes.Void().serializer(), Serdes.String().serializer())) {
      for (int i = 0; i < numRecords; i++) {
        long timestamp = computeTimestamp(type, baseTimestamp, (i % 3) - 1);
        String value = getValue(converter, topic, i);
        producer
            .send(new ProducerRecord<>(topic, null, timestamp, null, value))
  ...
  public static long computeTimestamp(
      TimePartitioning.Type partitioningType, long testStartTime, long shiftAmount) {
    long partitionDelta;
