...
public class StorageWriteApiCdcBigQuerySinkConnectorIT extends BaseConnectorIT {
  ...
  private static final Logger logger = LoggerFactory.getLogger(StorageWriteApiCdcBigQuerySinkConnectorIT.class);
  ...
  private void createTableWithPrimaryKey(String table, com.google.cloud.bigquery.Schema tableSchema, String... keyColumns) {
    ...
    PrimaryKey primaryKey = PrimaryKey.newBuilder()
        .setColumns(Arrays.asList(keyColumns))
        .build();
    ...
    try {
      ...
      bigQuery.query(com.google.cloud.bigquery.QueryJobConfiguration.of(String.format(
  ...
  public void testStorageWriteApiNativeCdc() throws Throwable {
    ...
    connect.kafka().produce(topic, key(keyConverter, topic, 1L), value(valueConverter, topic, "original value", false));
    
    // 2. Insert record for key 2
    connect.kafka().produce(topic, key(keyConverter, topic, 2L), value(valueConverter, topic, "other row", false));
    
    // 3. Update record for key 1
    connect.kafka().produce(topic, key(keyConverter, topic, 1L), value(valueConverter, topic, "modified value", false));
    
    // 4. Delete record for key 1 (tombstone)
    connect.kafka().produce(topic, key(keyConverter, topic, 1L), value(valueConverter, topic, null, true));
    ...
    // The query returns fields in order of table schema: k1, f1, _CHANGE_TYPE, _CHANGE_SEQUENCE_NUMBER
    // Since BigQuery CDC merges base table with changes, it should return 1 row.
    // Note: BQ CDC columns (_CHANGE_TYPE, _CHANGE_SEQUENCE_NUMBER) are system columns and we don't assert their exact values here if we only want the materialized state, 
  ...
  public void testPrintStreamSchema() throws Exception {
    ...
    com.google.cloud.bigquery.storage.v1.BigQueryWriteSettings settings = new com.wepay.kafka.connect.bigquery.GcpClientBuilder.BigQueryWriteSettingsBuilder()
        .withKey(keyFile())
        .withKeySource(com.wepay.kafka.connect.bigquery.GcpClientBuilder.KeySource.valueOf(keySource()))
        .withProject(project())
        .withWriterApi(true)
        .build();
    try (com.google.cloud.bigquery.storage.v1.BigQueryWriteClient client = com.google.cloud.bigquery.storage.v1.BigQueryWriteClient.create(settings)) {
      com.google.cloud.bigquery.storage.v1.WriteStream writeStream = client.getWriteStream(
